...
public class ClipZoomImageView extends ImageView implements
				...
				new SimpleOnGestureListener() {
					...
					public boolean onDoubleTap(MotionEvent e) {
						...
						if (getScale() < SCALE_MID) {
							ClipZoomImageView.this.postDelayed(new AutoScaleRunnable(SCALE_MID, x, y), 15);
							isAutoScale = true;
						} else {
							ClipZoomImageView.this.postDelayed(new AutoScaleRunnable(initScale, x, y), 15);
							isAutoScale = true;
						}
						return true;
					}
	...
	public void onGlobalLayout() {
		if (once) {
			...
			float scale = 1.0f;
			if (dw > dh) {
				scale = (getWidth() * 1.0f - mHorizontalPadding * 2) / dh;
			} else {
	...
	public Bitmap clip() {
		...
		try {
			...
			Canvas canvas = new Canvas(bitmap);
			draw(canvas);
			result = Bitmap.createBitmap(bitmap, mHorizontalPadding, mVerticalPadding,
