...
export default class Calendar extends Component {
  ...
  componentWillReceiveProps(props) {
    if (props.selectedDate) {
      this.setState({
        selectedMoment: props.selectedDate
      });
    }
  }
  ...
  selectDate(date) {
    //判断当前天是否可选
    let currentDay = moment().add(1, "days").format('YYYY-MM-DD');
    let preSixtyDay = moment().subtract(42, "days").format("YYYY-MM-DD");
    if (!moment(date).isBetween(preSixtyDay, currentDay)) {
      return;
    }
    this.setState({
      selectedMoment: date
  ...
  scrollEnded(event) {
    ...
    //滑动到第几月！！！
    this.props.updateMonth(moment(newMoment).format("MM"))
  }
  ...
  renderMonthView(argMoment, eventsMap) {
    ...
    do {
      ...
      if (dayIndex >= 0 && dayIndex < argMonthDaysCount) {
        const dayColor = this.isCanClick(moment(startOfArgMonthMoment).set('date', dayIndex + 1)) ? {
          day: {
            color: '#383838'
          }
        } : {
        ...
        days.push(( < Day startOfMonth = {
            startOfArgMonthMoment
          }
          isWeekend = {
            isoWeekday === 0 || isoWeekday === 6
          }
          key = {
            `${renderIndex}`
          }
          ...
          isToday = {
            argMonthIsToday && (dayIndex === todayIndex)
          }
          isSelected = {
            selectedMonthIsArg && (dayIndex === selectedIndex)
          }
          event = {
            events && events[dayIndex]
          }
          showEventIndicators = {
            this.props.showEventIndicators
          }
          customStyle = {
            dayColor
          }
  ...
  renderHeading() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      headings.push( < Text key = {
          i
        }
        style = {
          j === 0 || j === 6 ?
          [styles.weekendHeading, this.props.customStyle.weekendHeading] : [styles.dayHeading, this.props.customStyle.dayHeading]
        } > {
